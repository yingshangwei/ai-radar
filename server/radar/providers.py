...
INSTRUCTIONS += """\n文章 supplemental=true 表示此前发布但尚未报道的补充材料，必须保留其真实发布时间与作者，
不能称作当日新发布。补充展示标签由服务器按实际引用添加，无需在生成内容中自行添加标签。"""
...
CONCISE_INSTRUCTIONS = """\n精简与排版：先说最重要的新事实。概述/summary_zh 通常 80–160 字，最多两小段；必要时保留事实限定条件。
key_points_zh 只保留 2–3 个互不重复的重点，每项通常 30–70 字；没有新增信息就少列，不能凑数。
why_it_matters_zh 用一句话说明实际影响，不重复概述。普通发言若信息很少，可进一步缩短。
段落之间用换行，正文可用 **加粗** 标记最多 1–3 个真正关键的短语；标题不加 Markdown。
避免背景铺垫、空泛赞美和重复免责声明。数字、否定、引用归属、成立条件和不确定性不能为精简而丢失。
这只适用于总结字段，原文与完整翻译必须保留完整内容及段落。"""
INSTRUCTIONS += CONCISE_INSTRUCTIONS
READING_INSTRUCTIONS += CONCISE_INSTRUCTIONS
