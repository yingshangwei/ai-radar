...
当前服务器版本为 `20260908-c95c8d4`。X 重点账号公平轮转与持久分页、日报启动补偿均保留：启动及每 5 分钟检查最近到期窗口，已有日报直接复用；每窗口最多 3 次尝试，失败后退避 30 分钟、2 小时。隔离发布测试 800 通过、6 项既有浏览器环境测试跳过，Ruff 通过。激活 `inv-j8d9bvgigi` 成功，15 张表、已有 X 进度、三份配置、九组历史和其他服务保持；到期日报已经存在，本次启动没有新建任务或调用模型。完整过程见 [部署记录](docs/DEPLOYMENT.md)。

补充选稿已随当前版本上线：后续日报可从回看期内补入此前未报道的消息，保留真实发布时间，按历史实际引用去重。每份补充最多 5 条，当前窗口优先并为补充保留少量位置；报道按引用显示“补充｜”或“含补充｜”。关闭 reading 时不再临时抓取未保存网页。此次仅发布代码，尚未以新生产日报验证补充选稿效果。

2026-09-07 23:42:51 UTC 公网只读状态复验：主消息 46 ready；47 份网页全文 13 ready、34 review_required，全文中文尚未全部完成；健康、401/403 权限隔离正常，无运行任务，余额告警为空、调度器开启。此轮没有模型调用、人工改写或手动请求 X；已有 X healthy 状态不等于新分页策略已完成全部覆盖。未获诊断依据的 AI 筛选规则与中文输出门禁保持未发布。
