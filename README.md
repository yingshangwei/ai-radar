...
当前服务器版本为 `20260908-a510803`，增加严格复用已有模型审核的机器校验复检：仅当原文 / 候选指纹及审核策略仍一致，既有修订与独立审计明确通过，且当前机器检查已无问题时更新校验状态，不改写译文或调用模型。隔离发布测试 856 通过、6 项既有浏览器环境测试跳过，Ruff 通过。激活 `inv-e8dau3gp1m` 成功，15 张表、已有 X 进度、三份配置、九组历史和其他服务保持；到期日报已经存在，本次启动没有新建任务。完整过程见 [部署记录](docs/DEPLOYMENT.md)。
