...
当前服务器版本为 `20260908-8d5753d`，新增[跨任务翻译调用记录](docs/TRANSLATION.md)：修订次数和原文/候选审核结果持久保存，同一被否决译文不能重复抽审，未知调用不能靠强制重试解禁；明确结束的网络故障和余额不足保留有界恢复，正确候选正常审核发布。全套测试 1006 通过、6 项既有浏览器测试跳过，Ruff 和独立审查通过。激活 `inv-n8des00x53` 成功，16 张业务表、十一组历史、私有配置及其他服务保持，本次没有新增任务或重译文章。开发者只修改通用代码与回归。完整过程见 [部署记录](docs/DEPLOYMENT.md)。
