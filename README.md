...
当前服务器版本为 `20260908-28301b6`，新增[摘要事实审核](docs/SUMMARY-REVIEW.md)：以保存的原始来源审核日报和网页解读，仅修订问题部分，重新审核通过后发布；私有候选、调用预算和进度跨任务保存。隔离发布测试 978 通过、6 项既有浏览器环境测试跳过，Ruff 通过。激活 `inv-e8dcp0gr5f` 成功，原 15 张表、已有 X 进度、三份配置、十组历史和其他服务保持，新增私有审核表为空；到期日报已经存在，本次启动没有新建任务。开发者不直接修改文章或候选。完整过程见 [部署记录](docs/DEPLOYMENT.md)。
