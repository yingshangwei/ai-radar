...
## 2026-09-18 授权与真实采集验收

两家供应商的已有凭据已配置到生产热加载文件，本地副本权限为 0600、继续被 Git 忽略。传输使用服务器生成的一次性 RSA-OAEP 密钥加密，完成后已删除传输私钥；没有将凭据放入 App、源码或命令输出。TwitterAPI.io 初始赠送 10,000 点，Apify 免费账户的月使用上限为 5 美元、达到上限暂停；未付款、未订阅、未开启自动充值。TwitterAPI.io 页面当前最低单次充值为 10 美元，已留给用户付款，这不改变服务端 8 美元/月的主采集预算。

实测发现 TwitterAPI.io 空搜索结果返回 `has_next_page=false, next_cursor=null`，已在 `d3f5879` 中兼容此明确终止的页面；仍拒绝“还有下一页但缺少游标”及错误类型，避免虚假推进覆盖进度。130 项相关测试和 Ruff 通过，修复已部署。

正式采集轮次于北京时间 02:17 完成：读取 89 条、新增 49 条，覆盖当轮 31/31 个关注账号最新窗口；含前置验证与正常定时采集，本次累计新增 57 条 X 信息。另有 2 个待补页窗口及运行中动态新增的 1 个账号留待后续轮次，因此来源显示 `partial`，不代表全部历史补采完成。App 使用的 `/v1/articles` 已返回 Anthropic、Tibo、Boris Cherny、马斯克等原帖，正文处理继续由服务端正常流程执行。

Apify 真实对照成功取得 1 条范围内帖子，运行账单 0.0003244578 美元，费用账本另预留 0.002 美元结果读取余量。验证后生产配置已设 `x_data.shadow_enabled=true`，每天至多一次、最多 3 个账号/60 条；通用配置模板仍默认关闭，以免未验证账户直接启动。验收时主采集累计估算 0.0201 美元、对照估算及预留 0.002325 美元，无未决付费请求；这是当前试跑记录，不是长期覆盖率或固定月账单承诺。

最终生产 manifest SHA-256 为 `6cd7e80170fb68cb208ca0be4d6e0c651e32e1985f00871f4741bb387e16731b`。两次切换均从保存进度的空闲点执行，仅重启 AI Radar API，其余服务和 8080 进程保持不变。本次无需新 APK/OTA；部署回执和脱敏验证记录在忽略目录 `dist/x-provider-auth-20260918/`。
