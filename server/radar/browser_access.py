...
async def browser_fallback(sessions, url, client=None):
    ...
    async with locks.setdefault(domain, asyncio.Lock()):
        with sessions() as session:
            ...
            if browser_paused(access) or (access and access.status in INTERVENTION_STATUSES):
                return None
        ...
        with sessions.begin() as session:
            ...
            if not access:
                ...
                session.add(access)
            if not browser_paused(access) and result["status"] not in {"blocked", "restricted", "too_large", "rate_limited"}:
                access.status = "ready" if result["status"] == "fetched" else result["status"]
                if result["status"] == "fetched":
                    access.enabled = True
    ...
    for domain, group in grouped.items():
        ...
        group.update(enabled=bool(state and state.enabled), access_status=state.status if state else "unverified",
                     browser_enabled=not browser_paused(state) and not (
                         state and state.status in INTERVENTION_STATUSES),
        ...
        for doc in missing:
            ...
            if status not in {"blocked", "restricted", "rate_limited", "too_large"} and state and (
                ...
                status = state.status
            if status in {"blocked", "restricted", "access_restricted", "too_large"}:
                action = "restricted"
            elif status == "auth_required":
                action = "login"
            elif status in {"unavailable", "unsupported", "rate_limited", "busy", "disabled"}:
                action = "retry"
            else:
        ...
        messages = {
            ...
            "access_restricted": "网站限制服务器访问或要求人机验证，可改用手机读取。",
