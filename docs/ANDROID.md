...
接入 Expo 官方热更新，旧版需先覆盖安装一次；之后兼容的界面、逻辑及静态资源自动下载、下次启动生效，设置页提供检查与立即应用入口。原生模块和权限变化仍需新基础包。发布流程、签名、原生指纹与回退边界见 [热更新说明](UPDATES.md)。

交付包 `dist/ai-radar-0.13.0-android.apk`，74,364,304 字节，SHA-256 `1d504d4d8ad039b50b7b91993db7a4dad3022deb00fbc77cbdf56ec18c31081a`。沿用原应用 ID 和签名；四 ABI、最低 API 24 / 目标 36，ZIP 完整性与 6 个已知私密值扫描通过。Android 0.12 覆盖升级保留连接，原生设置页已检查。iOS 接入共享配置，尚无新 IPA。

## 历史版本：0.12.0 / code 13

「你的雷达 → 模型用量」新增余额与额度：DeepSeek 余额、阿里云财务授权状态、Codex 主额度、其他模型额度和重置时间，支持刷新与失败保留历史结果。沿用原签名和设备连接，详情见 [模型用量](MODEL-USAGE.md)。

交付包 `dist/ai-radar-0.12.0-android.apk`，70,313,510 字节，SHA-256 `5630c488f0b259912ddcb2f1c8d38fe247322cc2b0749701754a12fc5de8b72f`。code 13、原证书 SHA-256 `b166858aae76fd673083b3fe3860ede1a1732d03d1995e671aa751008f0c89e9`、四 ABI、最低 API 24 / 目标 36，ZIP 完整性、构建 bundle 一致性及 6 个已知私密值扫描通过。iOS 共享资源导出成功，未生成新 IPA。

专用 API 36 模拟器实测覆盖安装保留 reader、真实账户余额与主额度、待授权提示、手动刷新、其他额度展开及滚动到 Token 统计。证据在忽略目录 `dist/account-status/`；模拟器已停止，没有操作微信。
...
早期版本已在 Android 16/API 36 ARM64 模拟器完成安装、云端读取、历史日报、文章详情、收藏和取消、覆盖更新保留登录，以及断网冷启动缓存阅读。当前交付包为本文开头的 `dist/ai-radar-0.12.0-android.apk`，本次升级验收范围见当前版本段落；这些历史验证不构成本轮登录保留证明。此前 0.2.1 / code 3 包及其余额告警验收保留为历史记录；0.2.0 的中文正文、原文切换与离线中英阅读验收同样保留。
