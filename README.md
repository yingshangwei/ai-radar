...
当前服务端补丁 `f4acd22` 已将翻译切换到阿里云百炼，保留 Codex 的技术复核与摘要能力。此前 `32724ad` 增加技术语境与逐项概念审计、正文和标题关联校对、可配置的 Codex 技术复核，以及公式保留；正式模型复核和当前部署指纹见 [验证记录](docs/VALIDATION.md)与[部署记录](docs/DEPLOYMENT.md)。历史技术中文按新版策略由服务器队列处理，阅读页面复用已通过缓存。
