...
当前线上 `c95c8d4` 已包含 `08b19a1` 的读取边界修复：`reading.enabled=false` 时，日报只使用已经保存的消息原文与可复用中文，不再临时下载官方网页正文。顶层 `enrich_official_articles` 仅为旧配置兼容保留，不能绕过 reading 开关；启用读取后，网页正文仍由统一读取流程先持久化，日报再复用。
