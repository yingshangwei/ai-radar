...
当前服务器版本为 `20260908-f185060`，已上线[持久任务队列与恢复](docs/TASK_RECOVERY.md)：采集、翻译和网页/日报处理独立执行，排队任务合并，长文在已保存阶段间分批续跑；15 秒心跳、60 秒资格巡检及有限退避重试。已恢复旧版积压意图，原文、译文与审核记录保持。服务端 1085 项测试通过、6 项既有跳过，App 34 项通过、1 项既有跳过；实际部署与续跑证据见 [部署记录](docs/DEPLOYMENT.md)。
