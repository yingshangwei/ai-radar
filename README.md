...
- [AI 行业与投资研究](docs/INDUSTRY.md)：复用同一仓库与 App，首批 22 个免费原始来源、三类行业主题、公司观察目录、原文证据版本、经审核的行业判断和历史对照；[付费渠道费用比较](docs/FINANCIAL-DATA-PRICING.md)供后续选择。
- 每日中文汇报、历史日报、信号详情、原始引用；无新增内容时明确说明来源覆盖与数据空缺。
- 可替换模型的持久化中英对照：采集时翻译、修订校对、独立语义审计，未通过时由服务器自动修正并复审；中文搜索与离线阅读复用缓存。发现问题修复服务器逻辑，不人工修改文章或译文。详见 [翻译配置](docs/TRANSLATION.md)。
- 技术术语按原文语境消歧，独立审核概念与中文表达；论文公式在采集、翻译中保留 TeX，App 详情离线排版上下标、分数与多行公式。
- [国内团队与重点作者](docs/CHINA-SOURCES.md)：覆盖 Qwen、DeepSeek、Kimi、MiniMax、GLM 官方 X，以及 Seed、DeepSeek、Kimi、MiniMax 官网公告；在既有请求预算内优先拉取关注账号的新发言。
...
- 百炼或 DeepSeek 余额不足时在手机首页、详情和设置持续提示；保存翻译进度，后续调用成功自动恢复。
...
当前服务器基线为 `20260909-1a0b133`，已接入 [Crawl4AI 正文提取](docs/CRAWL4AI.md)、[Codex 会话复用与动态发现](docs/DISCOVERY.md)，以及 `1d1e827` 的[国内团队公告与关注优先采集](docs/CHINA-SOURCES.md)。保留学界采集及[持久任务恢复](docs/TASK_RECOVERY.md)。采集、翻译、发现和网页/日报处理分别执行；自动试关注每天最多 2 个、同时最多 6 个，7 天到期，用户的关闭和保留选择优先。X 每轮总请求预算仍为 3，先补关注账号最新窗口，再进行广泛搜索，不能据此承诺全量覆盖。

当前服务端补丁 `4ee6b20` 已启用 Codex 三档路由，日常生成和修正使用 Sol medium，确认使用 Astra low，有充分证据的疑难争议才升级 Astra medium。此前 `f4acd22` 已将翻译切换到阿里云百炼，本次保留其分工；`32724ad` 增加技术语境与逐项概念审计、正文和标题关联校对、可配置的 Codex 技术复核，以及公式保留。正式模型复核和当前部署指纹见 [验证记录](docs/VALIDATION.md)与[部署记录](docs/DEPLOYMENT.md)。历史技术中文按服务器队列处理，阅读页面复用已通过缓存。
...
翻译支持阿里云百炼：`qwen3.7-flash` 生成初稿，`qwen-plus` 普通修订与独立审计，统一关闭思考输出；技术论文继续使用 Codex 复核。供应商切换复用已批准的译文、草稿和调用历史，不重置预算。接入、价格依据及历史 DeepSeek 配置见 [翻译机制](docs/TRANSLATION.md)。
...
Android 0.13.0 签名安装包位于本机 `dist/ai-radar-0.13.0-android.apk`，沿用原应用 ID 与签名，可覆盖安装一次以启用 [Expo 热更新](docs/UPDATES.md)。兼容的界面、逻辑和静态资源会自动下载、下次启动生效；设置页也可检查和立即应用。更新由腾讯云独立只读服务分发，包含离线签名、原生指纹检查、资源完整性验证及回退指令。新增原生模块、权限或 SDK 仍需新基础包。

保留厂商余额与 Codex 额度、按模型/功能/阶段的 Token 用量统计、离线公式及[网页采集中心](docs/WEB-AUTHORIZATION.md)。iOS 配置与共享代码已接入热更新并通过资源导出；本机缺完整 Xcode 和原生签名条件，尚未生成新 IPA 或发布 iOS OTA，详见 [IOS.md](docs/IOS.md)。Facebook 账号正在审核；Meta 官方新闻和 Meta 的 X 帖子不代表 Facebook 社交帖已接通。
