...
服务端实现 `1d1e827` 已作为补丁应用到 `20260909-1a0b133`，不需要 App 升级。全量服务端验证 1490 passed、18 skipped（可选浏览器环境相关）；Ruff 通过。关注优先测试在固定每轮 3 次请求内完成 28 个账号的最新窗口轮转，保留待补分页并恢复广泛发现。

腾讯云 stage `inv-e8evddgga1` 实测：Seed 9 条、DeepSeek 5 条、Kimi 19 条、MiniMax 12 条，均来自官方页面，未调用模型、未写文章。四个首页的最新原日期分别为 2026-08-05、2026-04-24、2026-07-16、2026-08-26；不代表各团队其他渠道近期没有发布。主机本地一次 Seed 网络连接失败，云端验证正常，失败仍由每轮正式采集自动重试。

安全交接 `inv-e8evgk0pb3` 已成功：等待实际模型调用释放后启用新代码，重启前 22 张表与 schema 原样保留，不做数据库迁移、内容修改或额外模型调用。Caddy、浏览器服务、已有 8080 服务与授权文件保持原样。启用后 manifest SHA256 为 `82d6907377f9eb4c73c70f0f268db438d0719c1928651c903d9e6eac20e0ecfd`。凭证与部署回执均保留在非 Git 私有目录。

正式采集验收 `inv-n8evj9gbtb`：任务 `f4e81ac4-72d5-46b0-b510-02e6a8589b65` 于 2026-09-09 05:23:38 UTC 完成。四个官网来源均 healthy，近期新增均为 0；X 实际用了 3/3 次请求，最新窗口覆盖从 18/28 前进到 21/28，剩余账号和分页留给自动轮转，未重复强制请求或重置额度。全来源本轮新增 1 条，不将这条误归因为新增 X 帖子（X 新增为 0）。采集后原网页/翻译队列继续运行。
