...
当前已验证部署的服务器版本为 `20260908-4d196af`，完整测试 594 通过、6 项既有浏览器集成测试跳过，Ruff 通过。本版支持初稿、修订和独立审计分别配置模型请求参数与输出预算，并限制单次 SDK 调用的整体时长。线上已为修订和审计启用 DeepSeek `thinking=enabled`、`reasoning_effort=high`、32768 token 预算，整体超时为 300 秒；初稿保持非推理模式及 12000 token。只升级服务器，App 无需重建。

配置切换只改约定的七个字段，14 张数据表、七组历史记录及其他配置保持不变。新配置下的正式单条错误恢复任务已于 2026-09-07 21:18:13 UTC 完成，唯一 `error` 转为 `review_required`，没有新增整篇通过或发布的译文。修订和独立审计各两次 DeepSeek Pro 调用均正常结束，并返回推理 token，确认推理模式真实生效；候选仍有未通过的质量问题，未继续重跑。数据库全量主消息 46 条中文 ready，47 份已保存网页正文为 13 ready、34 review_required、0 error，全文中文尚未全部补齐。

2026-09-07 21:19:44 UTC 的公网只读验证显示：App 默认近七天可见 42 篇消息及 43 个关联资源，资源为 13 ready、30 review_required；这是时间窗口内可见范围，与数据库全量分开统计。未通过的全文中文不暴露，401/403 权限隔离正常，余额告警为空、X healthy，调度器继续开启；Facebook 仍待授权。同期服务器只读验收确认原文、60 份既有 ready 缓存、所有未选译文、七组历史记录及当前配置保持不变，无运行中任务、翻译租约或授权窗口。实际部署与任务证据见 [验证记录](docs/VALIDATION.md)。
