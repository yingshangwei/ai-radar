...
采集任务把来源正文、中文译文和内容解读分别持久化。Android 和 iOS 共用的消息详情代码提供中文摘要、关键要点、关注价值，以及可展开的中英正文；手机刷新只调用读取接口，复用服务器已保存的结果，不触发模型调用。Android 0.6 已完成专用模拟器升级与前后台恢复验收；iOS 构建和运行交付状态见 [IOS.md](IOS.md)。

## App 0.6 自动更新

App 在前台每 30 秒检查服务状态。任务结束、消息数量、中文处理进度或来源采集成功时间变化时，自动刷新消息、日报、历史日报列表和相关详情；状态没有变化时不会反复刷新全部内容。返回前台会恢复过期内容的读取，离线转在线后即使状态计数相同，也会补刷新一次。详情始终使用最新查询结果，收藏成功后同步详情及手机本地缓存。

手机自动读取仍只在前台进行：每五分钟及返回前台检查一次，每轮最多查询 30 个已许可网站、读取三篇。超过 30 个网站时循环轮转，进度按服务地址保存在手机上，后续打开继续；暂停、撤销或待验证的网站被排除。网页采集与授权方式见 [WEB-AUTHORIZATION.md](WEB-AUTHORIZATION.md)。
