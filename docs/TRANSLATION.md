...
程序检查数字/版本、百分比、货币符号、链接、@账号、中文覆盖、明显删漏以及输出段落是否完整对应。允许千分位、中英文月份和已知数量单位的等价转换，例如 `13 million` 与 `1300 万`。英文月份需有日期上下文；表示“可能”的 May 和人名不凭空转为数字。EUR、GBP 与明确货币语境中的 euro/pound 词支持中文货币等价表示，重量 pounds 不按英镑处理，歧义仍交给独立语义审计；引用帖的账号/时间、链接和 `$5m` 一类紧凑金额用可还原占位符保护，普通英文金额描述仍须翻译。无法通过检查或模型仍有疑点的内容标记 `review_required`，不对外发布候选译文。模型校对不能保证零错误，产品保留原文与一手链接，便于核验。
