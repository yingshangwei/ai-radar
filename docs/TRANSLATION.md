...
```toml
[translation.technical_review_provider]
kind = "codex"
command = ["codex"]
timeout_seconds = 600
...
翻译、校对和独立审计都接收原始标题与有界正文上下文（最多 12,000 字符，长文选开头与结尾并注明省略），校对可参考只含文字的跨段候选；独立审计仅接收原文上下文和当前待审项，不传入其他中文候选、先前的批准、问题解释或思考记录。服务器要求按研究对象、数学量和算法条件选择词义；难以确定的专名保留英文，解读可用原文支持的说明帮助理解，译文不能补造定义。

技术文档采用 `zh-agent-context-v6`。已完成的官方论文摘要或含 TeX 的文档会由正常队列安排一次版本复核，保留原候选及审计历史。校对时先依据原文列出关键术语、逐字证据、概念类别与语境含义，再独立生成中文；该步骤不提供当前项的旧译文。服务器验证术语引文确实来自原文，独立审计只接收原文与候选，不接收这份校对推断记录。正文先完成校验，标题再根据原文与已审正文独立生成并审核，避免错误译名带偏判断。当前版本已完成的段落直接复用，失败的候选仍受原有纠正轮次和调用记录约束。

独立技术审计先输出 `concept_checks`，分别核对概念含义和中文语境是否明确，再给出总批准结论。引用必须逐字属于原文与当前候选；任一概念被否决或有未解决疑点，即使顶层误报 `approved=true`，服务器也阻止发布并进入原有的有界修订流程。原始矛盾回执完整保存，同一被否决候选不能再抽一次审核。术语难懂本身不构成拒绝理由；门槛针对概念混淆、普通英文漏译及日常直译造成的语义缺失。

审计输出的 ID 限定在本次输入；错 ID 或引文不对应时与 JSON 结构错误共用一次格式恢复机会，不重映射批准结果。已确认结束的 v4 错 ID 审计，在该候选无语义否决、无未知调用且仍有修订次数时，可由此次范围修复迁移；失败段落的已用修订次数和原回执完整保留，不能重置预算。

非技术缓存不批量重翻；除此处明确的协议恢复外，未完成或失败的旧流程仍使用原策略与原预算，未知调用不重放。队列每轮有界运行并继续轮转，不能以升级策略为由无变化重复抽审。
...
结构化输出使用临时隔离进程。CLI 超时时，适配器先终止本次进程组并回收父进程，再抛出带类型的超时；取消仍保持取消，不伪装成可重试故障。审计专注原文与译文的对应关系，不重复证明论文结论；机器已逐字核对的公式有独立标识。

对旧版本被包装为不透明错误的调用，运维必须核实原超时配置、本地 CLI 已终止且无返回输出，才可在单条正式复核中使用 `--confirm-stopped-cli-timeout CALL_ID --previous-cli-timeout SECONDS`。入口核对精确当前调用、完整终止记录、运行时长、禁用工具的适配器、无已返回输出/语义否决及剩余传输次数；保留旧 unknown 记录，只追加确认。它与正常网络故障共用重试上限，不重置修订轮次、不重新生成已存候选，也不能通过连续确认增加预算。只有租约过期或失去进程消息的记录不符合条件。
