...
Android 0.10.0 签名安装包位于本机 `dist/ai-radar-0.10.0-android.apk`，沿用原应用 ID 与签名，可覆盖安装；新增离线公式显示，覆盖升级、阅读连接保留和正文滑动已在 Android 模拟器验证。[网页采集中心](docs/WEB-AUTHORIZATION.md)支持公开网页自动读取、按网站一次手机许可及 App 前台自动补采；包含网页与文章解读、中文阅读、原文切换、离线缓存和余额不足提示。iOS 共享代码与 0.10.0 资源导出通过；原生构建条件见 [IOS.md](docs/IOS.md)，本机缺完整 Xcode 和运行时，尚未实际运行 iOS 或生成真机 IPA。Facebook 账号正在审核；Meta 官方新闻和 Meta 的 X 帖子不代表 Facebook 社交帖已接通。
